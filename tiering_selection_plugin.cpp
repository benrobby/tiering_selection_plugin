...
namespace opossum
{
    ...
    void handle_run_calibration(const std::string command)
    {
        ...
        Assert(command_strings.size() >= num_commands,
               "Expecting one param. Usage: RUN TIERING CALIBRATION <file> <scale_factor> <benchmark_min_time_seconds> <random_data_size_per_device_mb> <monotonic_access_stride> <num_concurrent_threads> <num_reader_threads> <modes> <access_patterns> <device_1> <device_2> ...");
        ...
        const auto num_reader_threads = command_strings[9];
